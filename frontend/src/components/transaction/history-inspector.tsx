...
type HistoryInspectorAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: HistoryInspectorRecord["status"] }
  | { type: "note"; id: string; note: string };

const HistoryInspectorInitial: HistoryInspectorRecord[] = [
  { id: "a1", label: "Transaction", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "History", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Inspector", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Deep", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Detail", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: HistoryInspectorRecord[], action: HistoryInspectorAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: HistoryInspectorRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: HistoryInspectorRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: HistoryInspectorRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as HistoryInspectorRecord["status"])}
...
function NotesPanel({
  ...
  selected: HistoryInspectorRecord[];
...
export default function HistoryInspector() {
  const [rows, dispatch] = useReducer(reducer, HistoryInspectorInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build a Transaction History Inspector with Deep Detail Drawer and Searchable Receipts"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create an advanced history page that lets users search, inspect, and audit past transactions. * Search transactions by hash, amount, or address * Open a detail drawer with expanded metadata * Highlight failed and replaced transactions * Support pagination or virtualized lists * Keep the row and drawer components reusable"</p>
      </header>
