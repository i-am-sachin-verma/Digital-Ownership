...
type CrosschainRoutingSide = "left" | "right";

type CrosschainRoutingChunk = {
  id: string;
  label: string;
  left: string;
  right: string;
...
const CrosschainRoutingChunks: CrosschainRoutingChunk[] = [
  {
    id: "overview",
    label: "Cross / Chain",
...
export default function CrosschainRouting() {
  const [selected, setSelected] = useState("overview");
  const [compact, setCompact] = useState(false);

  const chunks = useMemo(() => {
    return CrosschainRoutingChunks.map((chunk) => {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Develop a Cross-Chain Transaction Routing UI with Network Selection and Path Comparison"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a routing interface that helps users choose between transaction paths across networks with cost and speed context. * Show multiple route options * Compare time and fee estimates * Support network-specific constraints * Handle unavailable routes gracefully * Keep the route card structure reusable"</p>
      </header>
