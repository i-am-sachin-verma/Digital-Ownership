...
type SimulationPreviewSide = "left" | "right";

type SimulationPreviewChunk = {
  id: string;
  label: string;
  left: string;
  right: string;
...
const SimulationPreviewChunks: SimulationPreviewChunk[] = [
  {
    id: "overview",
...
export default function SimulationPreview() {
  ...
  const chunks = useMemo(() => {
    return SimulationPreviewChunks.map((chunk) => {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Design a Transaction Simulation Preview with Before-Submit State Comparison"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a preview screen that shows how balances, approvals, and ownership will change after a transaction. * Compare before and after states * Highlight expected asset movement * Support simulation warnings and notes * Display clear confirmations for irreversible actions * Keep the preview layout modular and reusable"</p>
      </header>
