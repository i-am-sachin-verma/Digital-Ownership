...
type BalanceEditorAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: BalanceEditorRecord["status"] }
  | { type: "note"; id: string; note: string };

const BalanceEditorInitial: BalanceEditorRecord[] = [
  { id: "a1", label: "Wallet", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Balance", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Allocation", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Editor", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Transfer", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: BalanceEditorRecord[], action: BalanceEditorAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: BalanceEditorRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: BalanceEditorRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: BalanceEditorRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as BalanceEditorRecord["status"])}
...
export default function BalanceEditor() {
  const [rows, dispatch] = useReducer(reducer, BalanceEditorInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build a Wallet Balance Allocation Editor with Transfer Limits and Safety Warnings"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a transfer interface that lets users allocate balances while respecting limits and safety constraints. * Validate transferable balance in real time * Show warnings for risky or unusual values * Support percentage-based quick entry * Preserve input formatting and precision * Keep the editor reusable for send and bridge flows"</p>
      </header>
