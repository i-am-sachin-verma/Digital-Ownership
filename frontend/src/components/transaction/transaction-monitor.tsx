...
type TransactionMonitorAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: TransactionMonitorRecord["status"] }
  | { type: "note"; id: string; note: string };

const TransactionMonitorInitial: TransactionMonitorRecord[] = [
  { id: "a1", label: "Transaction", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Status", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Monitor", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Pending", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Queue", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: TransactionMonitorRecord[], action: TransactionMonitorAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: TransactionMonitorRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: TransactionMonitorRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: TransactionMonitorRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as TransactionMonitorRecord["status"])}
...
function NotesPanel({
  ...
  selected: TransactionMonitorRecord[];
...
export default function TransactionMonitor() {
  const [rows, dispatch] = useReducer(reducer, TransactionMonitorInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Implement a Transaction Status Monitor with Pending Queue, Success Receipt, and Failure Recovery"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a live transaction monitoring view that tracks submission progress and recovery actions. * Show queued, pending, confirmed, and failed states * Include clear recovery actions for failures * Display transaction hash and timestamps * Support polling-friendly UI updates * Keep the status component reusable across flows"</p>
      </header>
