...
type SecurityBannerAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: SecurityBannerRecord["status"] }
  | { type: "note"; id: string; note: string };

const SecurityBannerInitial: SecurityBannerRecord[] = [
  { id: "a1", label: "Transaction", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Security", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Review", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Banner", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Risk", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: SecurityBannerRecord[], action: SecurityBannerAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: SecurityBannerRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: SecurityBannerRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: SecurityBannerRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as SecurityBannerRecord["status"])}
...
export default function SecurityBanner() {
  const [rows, dispatch] = useReducer(reducer, SecurityBannerInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Create a Transaction Security Review Banner with Risk Labels and Consent Messaging"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Build a security-focused component that flags suspicious transactions and explains user-facing risk clearly. * Display risk labels and trust signals * Explain unusual contract behavior * Support safe action and cancel action states * Show concise consent messaging before approval * Keep the alert component reusable across sensitive flows"</p>
      </header>
