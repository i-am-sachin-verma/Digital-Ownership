...
type GasEstimatorAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: GasEstimatorRecord["status"] }
  | { type: "note"; id: string; note: string };

const GasEstimatorInitial: GasEstimatorRecord[] = [
  { id: "a1", label: "Gas", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Fee", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Estimation", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Tiered", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Speed", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: GasEstimatorRecord[], action: GasEstimatorAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: GasEstimatorRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: GasEstimatorRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: GasEstimatorRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as GasEstimatorRecord["status"])}
...
export default function GasEstimator() {
  const [rows, dispatch] = useReducer(reducer, GasEstimatorInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Develop a Gas Fee Estimation Panel with Tiered Speed Options and Live Cost Breakdown"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create an interface that helps users understand estimated gas costs and choose between fee tiers. * Show slow, standard, and fast options * Break down total cost clearly * Update estimates when inputs change * Handle uncertain or unavailable fee data gracefully * Keep the panel reusable for all transaction pages"</p>
      </header>
