...
function SectionHeader({ title, description }: Pick<NFTGalleryGridProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "NFT Gallery Grid"}</h2>
...
export interface NFTGalleryGridProps extends NFTGalleryGridProps {
  items?: DataItem[];
}
...
export const NFTGalleryVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const NFTGallerySizes = ["sm", "md", "lg"] as const;
export function NFTGalleryIsInteractive(disabled?: boolean) { return !disabled; }
export function NFTGalleryDisplayLabel(index: number) { return "NFTGallery item " + String(index + 1); }
