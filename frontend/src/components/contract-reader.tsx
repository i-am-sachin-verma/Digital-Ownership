...
export interface ContractReadPanelProps {
  className?: string;
  title?: string;
  description?: string;
  disabled?: boolean;
...
function SectionHeader({ title, description }: Pick<ContractReadPanelProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "Contract Read Panel"}</h2>
...
export interface ContractReadPanelProps extends ContractReadPanelProps {
  items?: DataItem[];
}

export default function ContractReadPanel({
  title = "Contract Read Panel",
  description = "Reusable web3 frontend component.",
  items = sampleItems,
  className,
  variant = "default",
  size = "md",
}: ContractReadPanelProps) {
...
export const ContractReadVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const ContractReadSizes = ["sm", "md", "lg"] as const;
export function ContractReadIsInteractive(disabled?: boolean) { return !disabled; }
export function ContractReadDisplayLabel(index: number) { return "ContractRead item " + String(index + 1); }
