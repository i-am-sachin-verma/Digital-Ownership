...
const InsightCenterEvents: InsightCenterEvent[] = [
  {
    id: "e1",
    title: "Notification ready for review",
    kind: "proposal",
    time: "2m ago",
    unread: true,
    archived: false,
  ...
  {
    id: "e2",
    title: "Insight action completed",
    kind: "vote",
    time: "15m ago",
    unread: true,
    archived: false,
...
function kindTone(kind: InsightCenterEvent["kind"]) {
  switch (kind) {
    case "proposal":
...
function EventBadge({ kind }: { kind: InsightCenterEvent["kind"] }) {
  return (
...
export default function InsightCenter() {
  const [filter, setFilter] = useState<InsightCenterEvent["kind"] | "all">("all");
  const [showArchived, setShowArchived] = useState(true);
  const [items, setItems] = useState(InsightCenterEvents);
  ...
  return (
    <section className="mx-auto grid max-w-5xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Develop a Notification and Insight Center with Unified Feed and Smart Categorization"</h2>
        <p className="max-w-3xl text-sm text-slate-600">"Create a dashboard inbox that combines alerts, updates, and insights into one structured feed. * Categorize updates automatically * Support read, unread, and archived states * Display insight cards alongside notifications * Handle density without overwhelming the user * Keep the feed item component reusable"</p>
      </header>
