...
type WidgetLibraryLayoutState = {
  panels: WidgetLibraryPanel[];
  selected: string | null;
  compact: boolean;
};

type WidgetLibraryContextValue = {
  state: WidgetLibraryLayoutState;
...
const WidgetLibraryPanels: WidgetLibraryPanel[] = [
  { id: "p1", title: "Modular", tone: "neutral", span: "lg" },
  { id: "p2", title: "Widget", tone: "positive", span: "md" },
  { id: "p3", title: "Library", tone: "warning", span: "sm" },
  { id: "p4", title: "Drag", tone: "neutral", span: "md" },
  { id: "p5", title: "Place", tone: "positive", span: "sm" },
  { id: "p6", title: "Configurable", tone: "warning", span: "lg" },
];

const WidgetLibraryContext = createContext<WidgetLibraryContextValue | null>(null);

function useWidgetLibraryLayout() {
  const context = useContext(WidgetLibraryContext);
  if (!context) {
    throw new Error("WidgetLibrary must be used inside its provider.");
  }
  return context;
}

function scoreSpan(span: WidgetLibraryPanel["span"]) {
  if (span === "lg") return "col-span-2";
  if (span === "md") return "col-span-1";
  return "col-span-1";
}

function toneStyles(tone: WidgetLibraryPanel["tone"]) {
  if (tone === "positive") return "bg-emerald-50 text-emerald-700";
  if (tone === "warning") return "bg-amber-50 text-amber-700";
  return "bg-slate-100 text-slate-600";
}

function PanelCard({ panel }: { panel: WidgetLibraryPanel }) {
  const { state, movePanel, selectPanel } = useWidgetLibraryLayout();
...
function Toolbar() {
  const { state, toggleCompact, resetLayout } = useWidgetLibraryLayout();
  return (
...
function Provider({
  ...
  const [state, setState] = useState<WidgetLibraryLayoutState>({
    panels: WidgetLibraryPanels,
    selected: null,
    compact: false,
  });

  useEffect(() => {
    const raw = window.localStorage.getItem("WidgetLibrary:layout");
    if (!raw) return;

    try {
      const parsed = JSON.parse(raw) as Partial<WidgetLibraryLayoutState>;
      setState((current) => ({
        ...current,
  ...
  useEffect(() => {
    window.localStorage.setItem("WidgetLibrary:layout", JSON.stringify(state));
  }, [state]);
  ...
  const api = useMemo<WidgetLibraryContextValue>(() => {
    return {
      state,
  ...
  return <WidgetLibraryContext.Provider value={api}>{children}</WidgetLibraryContext.Provider>;
}
    ...
    <Provider>
      <section className="mx-auto grid max-w-7xl gap-6 p-6">
        <header className="grid gap-2">
          ...
          <h2 className="text-2xl font-semibold text-slate-900">"Build a Modular Dashboard Widget Library with Drag-to-Place and Configurable Slots"</h2>
          <p className="max-w-4xl text-sm text-slate-600">"Create a widget library interface where users can pick, preview, and place components into configurable dashboard slots. * Allow widget browsing and previewing * Support drag-to-place interactions * Show slot compatibility states * Persist selected widget sets * Keep widget cards reusable across modules"</p>
        </header>

        <Toolbar />

        <div className="grid gap-4 lg:grid-cols-3">
          {WidgetLibraryPanels.map((panel) => (
