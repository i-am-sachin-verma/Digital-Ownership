...
type ExecutiveSummaryRange = "24h" | "7d" | "30d" | "90d";

const ExecutiveSummarySeries: ExecutiveSummaryMetric[] = [
  { id: "m1", label: "Compact", value: 90, delta: 1.5, color: "neutral", format: "number" },
  { id: "m2", label: "Executive", value: 103, delta: 2.7, color: "positive", format: "currency" },
  { id: "m3", label: "Summary", value: 116, delta: 3.9, color: "negative", format: "percent" },
  { id: "m4", label: "Kpi", value: 129, delta: 5.1, color: "neutral", format: "number" },
  { id: "m5", label: "Cards", value: 142, delta: 6.3, color: "neutral", format: "number" },
  { id: "m6", label: "Deep", value: 155, delta: 7.5, color: "positive", format: "currency" },
...
function formatMetric(value: number, format: ExecutiveSummaryMetric["format"]) {
  if (format === "currency") {
...
function RangePill({
  ...
  value: ExecutiveSummaryRange;
  active: boolean;
  onSelect: (value: ExecutiveSummaryRange) => void;
}) {
  return (
    <button
      type="button"
...
export default function ExecutiveSummary() {
  const [range, setRange] = useState<ExecutiveSummaryRange>("30d");
  ...
  const rows = useMemo(() => {
    ...
    return ExecutiveSummarySeries.map((metric, index) => ({
      ...metric,
  ...
  return (
    <section ref={anchorRef} className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build a Compact Executive Summary Dashboard with KPI Cards and Deep Drilldown Entry Points"</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          "Create a concise summary screen that presents top-level metrics with fast access to deeper module-specific details. * Show essential KPI cards only * Provide drilldown links per metric * Support compact and expanded density modes * Display trend markers and deltas clearly * Keep the summary blocks reusable"
        </p>
      </header>
