...
type OverviewPanelRange = "24h" | "7d" | "30d" | "90d";

const OverviewPanelSeries: OverviewPanelMetric[] = [
  { id: "m1", label: "Smart", value: 90, delta: 1.5, color: "neutral", format: "number" },
  { id: "m2", label: "Overview", value: 103, delta: 2.7, color: "positive", format: "currency" },
  { id: "m3", label: "Threshold", value: 116, delta: 3.9, color: "negative", format: "percent" },
  { id: "m4", label: "Alerts", value: 129, delta: 5.1, color: "neutral", format: "number" },
  { id: "m5", label: "Data", value: 142, delta: 6.3, color: "neutral", format: "number" },
  { id: "m6", label: "Driven", value: 155, delta: 7.5, color: "positive", format: "currency" },
...
function formatMetric(value: number, format: OverviewPanelMetric["format"]) {
  if (format === "currency") {
...
function RangePill({
  ...
  value: OverviewPanelRange;
  active: boolean;
  onSelect: (value: OverviewPanelRange) => void;
}) {
  return (
    <button
      type="button"
...
export default function OverviewPanel() {
  const [range, setRange] = useState<OverviewPanelRange>("30d");
  ...
  const rows = useMemo(() => {
    ...
    return OverviewPanelSeries.map((metric, index) => ({
      ...metric,
  ...
  return (
    <section ref={anchorRef} className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Design a Smart Overview Panel with Threshold Alerts and Data-Driven Highlighting"</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          "Create a dashboard panel that surfaces important values through threshold-based highlighting and alert states. * Highlight critical metrics automatically * Support custom alert thresholds * Display positive and negative changes clearly * Handle both summary and detailed views * Keep alert logic presentation-ready and reusable"
        </p>
      </header>
