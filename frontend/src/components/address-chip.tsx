...
function SectionHeader({ title, description }: Pick<AddressChipProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "Address Chip"}</h2>
...
export interface AddressChipProps extends AddressChipProps {
  items?: DataItem[];
}
...
export const AddressChipVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const AddressChipSizes = ["sm", "md", "lg"] as const;
export function AddressChipIsInteractive(disabled?: boolean) { return !disabled; }
export function AddressChipDisplayLabel(index: number) { return "AddressChip item " + String(index + 1); }
