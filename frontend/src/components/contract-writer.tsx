...
export interface ContractWriteFormProps {
  className?: string;
  title?: string;
  description?: string;
  disabled?: boolean;
...
function SectionHeader({ title, description }: Pick<ContractWriteFormProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "Contract Write Form"}</h2>
...
export interface ContractWriteFormProps extends ContractWriteFormProps {
  items?: DataItem[];
}

export default function ContractWriteForm({
  title = "Contract Write Form",
  description = "Reusable web3 frontend component.",
  items = sampleItems,
  className,
  variant = "default",
  size = "md",
}: ContractWriteFormProps) {
...
export const ContractWriteVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const ContractWriteSizes = ["sm", "md", "lg"] as const;
export function ContractWriteIsInteractive(disabled?: boolean) { return !disabled; }
export function ContractWriteDisplayLabel(index: number) { return "ContractWrite item " + String(index + 1); }
