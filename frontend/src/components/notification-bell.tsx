...
function SectionHeader({ title, description }: Pick<NotificationBellProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "Notification Bell"}</h2>
...
export interface NotificationBellProps extends NotificationBellProps {
  items?: DataItem[];
}
...
export const NotificationBellVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const NotificationBellSizes = ["sm", "md", "lg"] as const;
export function NotificationBellIsInteractive(disabled?: boolean) { return !disabled; }
export function NotificationBellDisplayLabel(index: number) { return "NotificationBell item " + String(index + 1); }
