...
function SectionHeader({ title, description }: Pick<VoteSelectorProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "Vote Selector"}</h2>
...
export interface VoteSelectorProps extends VoteSelectorProps {
  items?: DataItem[];
}
...
export const VoteSelectorVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const VoteSelectorSizes = ["sm", "md", "lg"] as const;
export function VoteSelectorIsInteractive(disabled?: boolean) { return !disabled; }
export function VoteSelectorDisplayLabel(index: number) { return "VoteSelector item " + String(index + 1); }
