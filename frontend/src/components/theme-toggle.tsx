...
export interface ThemeToggleDockProps {
  className?: string;
  title?: string;
  description?: string;
  disabled?: boolean;
...
function SectionHeader({ title, description }: Pick<ThemeToggleDockProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "Theme Toggle Dock"}</h2>
...
export interface ThemeToggleDockProps extends ThemeToggleDockProps {
  items?: DataItem[];
}

export default function ThemeToggleDock({
  title = "Theme Toggle Dock",
  description = "Reusable web3 frontend component.",
  items = sampleItems,
  className,
  variant = "default",
  size = "md",
}: ThemeToggleDockProps) {
...
export const ThemeToggleVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const ThemeToggleSizes = ["sm", "md", "lg"] as const;
export function ThemeToggleIsInteractive(disabled?: boolean) { return !disabled; }
export function ThemeToggleDisplayLabel(index: number) { return "ThemeToggle item " + String(index + 1); }
