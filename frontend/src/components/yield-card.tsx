...
export interface YieldFarmCardProps {
  className?: string;
  title?: string;
  description?: string;
  disabled?: boolean;
...
function SectionHeader({ title, description }: Pick<YieldFarmCardProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "Yield Farm Card"}</h2>
...
export interface YieldFarmCardProps extends YieldFarmCardProps {
  items?: DataItem[];
}

export default function YieldFarmCard({
  title = "Yield Farm Card",
...
export const YieldFarmVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const YieldFarmSizes = ["sm", "md", "lg"] as const;
export function YieldFarmIsInteractive(disabled?: boolean) { return !disabled; }
export function YieldFarmDisplayLabel(index: number) { return "YieldFarm item " + String(index + 1); }
