...
export interface OnchainStatsTickerProps {
  className?: string;
  title?: string;
  description?: string;
  disabled?: boolean;
...
function SectionHeader({ title, description }: Pick<OnchainStatsTickerProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "On-chain Stats Ticker"}</h2>
...
export interface OnchainStatsTickerProps extends OnchainStatsTickerProps {
  items?: DataItem[];
}

export default function OnchainStatsTicker({
  title = "On-chain Stats Ticker",
  description = "Reusable web3 frontend component.",
  items = sampleItems,
  className,
  variant = "default",
  size = "md",
}: OnchainStatsTickerProps) {
...
export const OnchainStatsVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const OnchainStatsSizes = ["sm", "md", "lg"] as const;
export function OnchainStatsIsInteractive(disabled?: boolean) { return !disabled; }
export function OnchainStatsDisplayLabel(index: number) { return "OnchainStats item " + String(index + 1); }
