...
type NftCompareSide = "left" | "right";

type NftCompareChunk = {
  id: string;
  label: string;
  left: string;
  right: string;
...
const NftCompareChunks: NftCompareChunk[] = [
  {
    id: "overview",
...
export default function NftCompare() {
  const [selected, setSelected] = useState("overview");
  const [compact, setCompact] = useState(false);

  const chunks = useMemo(() => {
    return NftCompareChunks.map((chunk) => {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build a Multi-Collection NFT Comparison View with Media, Rarity, and Utility Columns"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a comparison layout that helps users evaluate multiple NFT collections by useful trading and utility criteria. * Compare collections in structured columns * Include media and rarity summaries * Show utility, floor, and volume signals * Support horizontal scrolling on smaller screens * Keep the table layout reusable"</p>
      </header>
