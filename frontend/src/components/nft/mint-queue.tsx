...
type MintQueueAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: MintQueueRecord["status"] }
  | { type: "note"; id: string; note: string };

const MintQueueInitial: MintQueueRecord[] = [
  { id: "a1", label: "Nft", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Mint", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Queue", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Priority", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "States", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: MintQueueRecord[], action: MintQueueAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: MintQueueRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  onStatus: (status: MintQueueRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as MintQueueRecord["status"])}
...
export default function MintQueue() {
  const [rows, dispatch] = useReducer(reducer, MintQueueInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Develop an NFT Mint Queue Interface with Priority States and Wallet Session Feedback"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a mint queue component that shows mint order, processing state, and session health for connected wallets. * Show queue position and estimated processing * Reflect wallet connection status clearly * Support waiting, success, and failure states * Provide clear retry and refresh actions * Keep queue logic presentation-focused and reusable"</p>
      </header>
