...
const OwnershipShowcaseItems: OwnershipShowcaseItem[] = [
  { id: "i1", name: "Nft Alpha", price: 14.2, rarity: 94, chain: "Base", featured: true, listed: true },
  { id: "i2", name: "Ownership Prime", price: 9.8, rarity: 81, chain: "Ethereum", featured: false, listed: true },
  { id: "i3", name: "Showcase Core", price: 31.5, rarity: 76, chain: "Polygon", featured: true, listed: false },
  { id: "i4", name: "Wallet Vault", price: 5.4, rarity: 88, chain: "Arbitrum", featured: false, listed: true },
  { id: "i5", name: "Albums Signal", price: 18.9, rarity: 90, chain: "Optimism", featured: true, listed: true },
  { id: "i6", name: "Transfer Drift", price: 22.3, rarity: 73, chain: "Base", featured: false, listed: false },
];

function sortItems(items: OwnershipShowcaseItem[], mode: "price" | "rarity" | "featured") {
  const copy = [...items];
...
export default function OwnershipShowcase() {
  ...
  const visible = useMemo(() => {
    return sortItems(
      OwnershipShowcaseItems.filter((item) => {
  ...
  const chains = useMemo(() => {
    return Array.from(new Set(OwnershipShowcaseItems.map((item) => item.chain)));
  }, []);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Create an NFT Ownership Showcase with Wallet Albums and Transfer-Aware States"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Build a gallery-style ownership page that displays held NFTs with transfer and listing awareness. * Group NFTs by collection or status * Show listed, locked, and transferable states * Support grid and list display modes * Handle missing or broken media gracefully * Keep the gallery reusable across wallet screens"</p>
      </header>
