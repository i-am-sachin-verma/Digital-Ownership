...
type FloorDashboardRange = "24h" | "7d" | "30d" | "90d";

const FloorDashboardSeries: FloorDashboardMetric[] = [
  { id: "m1", label: "Floor", value: 90, delta: 1.5, color: "neutral", format: "number" },
  { id: "m2", label: "Tracking", value: 103, delta: 2.7, color: "positive", format: "currency" },
  { id: "m3", label: "Nft", value: 116, delta: 3.9, color: "negative", format: "percent" },
  { id: "m4", label: "Collections", value: 129, delta: 5.1, color: "neutral", format: "number" },
  { id: "m5", label: "Historical", value: 142, delta: 6.3, color: "neutral", format: "number" },
  { id: "m6", label: "Charting", value: 155, delta: 7.5, color: "positive", format: "currency" },
...
function formatMetric(value: number, format: FloorDashboardMetric["format"]) {
  if (format === "currency") {
...
function RangePill({
  ...
  value: FloorDashboardRange;
  active: boolean;
  onSelect: (value: FloorDashboardRange) => void;
}) {
  return (
    <button
      type="button"
...
export default function FloorDashboard() {
  const [range, setRange] = useState<FloorDashboardRange>("30d");
  ...
  const rows = useMemo(() => {
    ...
    return FloorDashboardSeries.map((metric, index) => ({
      ...metric,
  ...
  return (
    <section ref={anchorRef} className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Create a Floor Tracking Dashboard for NFT Collections with Historical Charting and Alerts"</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          "Build a collection dashboard that tracks floor price movement and surfaces changes through alert-style UI. * Show historical floor price trends * Add configurable alerts for major changes * Support multiple time ranges * Display volume and holder context * Keep chart and alert components reusable"
        </p>
      </header>
