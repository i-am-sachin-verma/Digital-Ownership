...
type MintStudioAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: MintStudioRecord["status"] }
  | { type: "note"; id: string; note: string };

const MintStudioInitial: MintStudioRecord[] = [
  { id: "a1", label: "Nft", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Minting", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Studio", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Trait", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Preview", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: MintStudioRecord[], action: MintStudioAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: MintStudioRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: MintStudioRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: MintStudioRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as MintStudioRecord["status"])}
...
export default function MintStudio() {
  const [rows, dispatch] = useReducer(reducer, MintStudioInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build an NFT Minting Studio with Trait Preview, Metadata Form, and Live Asset Rendering"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a minting interface that supports metadata entry, trait selection, and live preview of the NFT being created. * Allow editable metadata fields * Render a live preview of the NFT * Support trait combinations and validation * Show mint readiness and error states * Keep the studio layout reusable for different collections"</p>
      </header>
