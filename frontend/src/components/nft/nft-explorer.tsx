...
const NftExplorerItems: NftExplorerItem[] = [
  { id: "i1", name: "Nft Alpha", price: 14.2, rarity: 94, chain: "Base", featured: true, listed: true },
  { id: "i2", name: "Collection Prime", price: 9.8, rarity: 81, chain: "Ethereum", featured: false, listed: true },
  { id: "i3", name: "Explorer Core", price: 31.5, rarity: 76, chain: "Polygon", featured: true, listed: false },
  { id: "i4", name: "Trait Vault", price: 5.4, rarity: 88, chain: "Arbitrum", featured: false, listed: true },
  { id: "i5", name: "Filters Signal", price: 18.9, rarity: 90, chain: "Optimism", featured: true, listed: true },
  { id: "i6", name: "Rarity Drift", price: 22.3, rarity: 73, chain: "Base", featured: false, listed: false },
];

function sortItems(items: NftExplorerItem[], mode: "price" | "rarity" | "featured") {
  const copy = [...items];
...
export default function NftExplorer() {
  ...
  const chains = useMemo(() => {
    return Array.from(new Set(NftExplorerItems.map((item) => item.chain)));
  }, []);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Develop an NFT Collection Explorer with Trait Filters and Rarity-Weighted Sorting"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a collection browsing experience that helps users find assets using trait filters and rarity-aware ranking. * Filter by attributes and rarity tiers * Support multiple sort strategies * Show compact and expanded card layouts * Preserve performance for large collections * Keep filter and card components modular"</p>
      </header>
