...
type ListingManagerAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: ListingManagerRecord["status"] }
  | { type: "note"; id: string; note: string };

const ListingManagerInitial: ListingManagerRecord[] = [
  { id: "a1", label: "Nft", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Listing", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Manager", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Price", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Editing", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: ListingManagerRecord[], action: ListingManagerAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: ListingManagerRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: ListingManagerRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: ListingManagerRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as ListingManagerRecord["status"])}
...
export default function ListingManager() {
  const [rows, dispatch] = useReducer(reducer, ListingManagerInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build an NFT Listing Manager with Price Editing, Expiry Controls, and Status Sync"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a seller tool that manages NFT listings with pricing, expiry, and status updates. * Edit listing price and duration * Show sync status for active listings * Support cancel and relist actions * Validate ownership and availability before changes * Keep the listing row reusable"</p>
      </header>
