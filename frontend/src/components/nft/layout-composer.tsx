...
type LayoutComposerLayoutState = {
  panels: LayoutComposerPanel[];
  selected: string | null;
  compact: boolean;
};

type LayoutComposerContextValue = {
  state: LayoutComposerLayoutState;
...
const LayoutComposerPanels: LayoutComposerPanel[] = [
  { id: "p1", title: "Adaptive", tone: "neutral", span: "lg" },
  { id: "p2", title: "Layout", tone: "positive", span: "md" },
  { id: "p3", title: "Composer", tone: "warning", span: "sm" },
  { id: "p4", title: "Resizable", tone: "neutral", span: "md" },
  { id: "p5", title: "Content", tone: "positive", span: "sm" },
  { id: "p6", title: "Regions", tone: "warning", span: "lg" },
];

const LayoutComposerContext = createContext<LayoutComposerContextValue | null>(null);

function useLayoutComposerLayout() {
  const context = useContext(LayoutComposerContext);
  if (!context) {
    throw new Error("LayoutComposer must be used inside its provider.");
  }
  return context;
}

function scoreSpan(span: LayoutComposerPanel["span"]) {
  if (span === "lg") return "col-span-2";
  if (span === "md") return "col-span-1";
  return "col-span-1";
}

function toneStyles(tone: LayoutComposerPanel["tone"]) {
  if (tone === "positive") return "bg-emerald-50 text-emerald-700";
  if (tone === "warning") return "bg-amber-50 text-amber-700";
  return "bg-slate-100 text-slate-600";
}

function PanelCard({ panel }: { panel: LayoutComposerPanel }) {
  const { state, movePanel, selectPanel } = useLayoutComposerLayout();
...
function Toolbar() {
  const { state, toggleCompact, resetLayout } = useLayoutComposerLayout();
  return (
...
function Provider({
  ...
  const [state, setState] = useState<LayoutComposerLayoutState>({
    panels: LayoutComposerPanels,
    selected: null,
    compact: false,
  });

  useEffect(() => {
    const raw = window.localStorage.getItem("LayoutComposer:layout");
    if (!raw) return;

    try {
      const parsed = JSON.parse(raw) as Partial<LayoutComposerLayoutState>;
      setState((current) => ({
        ...current,
  ...
  useEffect(() => {
    window.localStorage.setItem("LayoutComposer:layout", JSON.stringify(state));
  }, [state]);
  ...
  const api = useMemo<LayoutComposerContextValue>(() => {
    return {
      state,
  ...
  return <LayoutComposerContext.Provider value={api}>{children}</LayoutComposerContext.Provider>;
}
    ...
    <Provider>
      <section className="mx-auto grid max-w-7xl gap-6 p-6">
        <header className="grid gap-2">
          ...
          <h2 className="text-2xl font-semibold text-slate-900">"Build an Adaptive Dashboard Layout Composer with Resizable Content Regions and Persistent Workspace State"</h2>
          <p className="max-w-4xl text-sm text-slate-600">"Create a dashboard layout system that lets users rearrange panels, resize regions, and keep their workspace state across sessions. * Support draggable and resizable sections * Persist layout preferences locally * Handle responsive behavior on smaller screens * Allow multiple panel arrangement modes * Keep the structure reusable for different dashboard types"</p>
        </header>

        <Toolbar />

        <div className="grid gap-4 lg:grid-cols-3">
          {LayoutComposerPanels.map((panel) => (
