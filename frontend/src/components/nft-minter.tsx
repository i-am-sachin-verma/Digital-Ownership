...
export interface NFTMintCardProps {
  className?: string;
  title?: string;
  description?: string;
  disabled?: boolean;
...
function SectionHeader({ title, description }: Pick<NFTMintCardProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "NFT Mint Card"}</h2>
...
export interface NFTMintCardProps extends NFTMintCardProps {
  items?: DataItem[];
}
...
export const NFTMintVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const NFTMintSizes = ["sm", "md", "lg"] as const;
export function NFTMintIsInteractive(disabled?: boolean) { return !disabled; }
export function NFTMintDisplayLabel(index: number) { return "NFTMint item " + String(index + 1); }
