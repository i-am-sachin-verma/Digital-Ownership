...
function SectionHeader({ title, description }: Pick<SecurityWarningBannerProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "Security Warning Banner"}</h2>
...
export interface SecurityWarningBannerProps extends SecurityWarningBannerProps {
  items?: DataItem[];
}

export default function SecurityWarningBanner({
  title = "Security Warning Banner",
...
export const SecurityWarningVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const SecurityWarningSizes = ["sm", "md", "lg"] as const;
export function SecurityWarningIsInteractive(disabled?: boolean) { return !disabled; }
export function SecurityWarningDisplayLabel(index: number) { return "SecurityWarning item " + String(index + 1); }
