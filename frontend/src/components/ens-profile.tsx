...
function SectionHeader({ title, description }: Pick<ENSProfileCardProps, "title" | "description">) {
  return (
    <header className="space-y-1">
      <h2 className="text-base font-semibold tracking-tight">{title ?? "ENS Profile Card"}</h2>
...
export interface ENSProfileCardProps extends ENSProfileCardProps {
  items?: DataItem[];
}

export default function ENSProfileCard({
  title = "ENS Profile Card",
...
export const ENSProfileVariants = ["default", "primary", "secondary", "success", "warning", "danger"] as const;
export const ENSProfileSizes = ["sm", "md", "lg"] as const;
export function ENSProfileIsInteractive(disabled?: boolean) { return !disabled; }
export function ENSProfileDisplayLabel(index: number) { return "ENSProfile item " + String(index + 1); }
