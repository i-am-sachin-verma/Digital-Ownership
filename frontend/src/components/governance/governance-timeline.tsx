...
const GovernanceTimelineEvents: GovernanceTimelineEvent[] = [
  {
    id: "e1",
    title: "Governance ready for review",
    kind: "proposal",
    time: "2m ago",
    unread: true,
    archived: false,
  ...
  {
    id: "e3",
    title: "Timeline triggered a sync",
    kind: "execution",
    time: "1h ago",
    unread: false,
    archived: false,
...
function kindTone(kind: GovernanceTimelineEvent["kind"]) {
  switch (kind) {
    case "proposal":
...
function EventBadge({ kind }: { kind: GovernanceTimelineEvent["kind"] }) {
  return (
...
export default function GovernanceTimeline() {
  const [filter, setFilter] = useState<GovernanceTimelineEvent["kind"] | "all">("all");
  const [showArchived, setShowArchived] = useState(true);
  const [items, setItems] = useState(GovernanceTimelineEvents);
  ...
  return (
    <section className="mx-auto grid max-w-5xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Implement a Governance Activity Timeline with Event Grouping and Advanced Filtering"</h2>
        <p className="max-w-3xl text-sm text-slate-600">"Create a timeline view for governance actions such as proposal creation, voting, execution, and delegation changes. * Group events by proposal lifecycle stage * Add filtering by event type and date range * Support compact and expanded timeline modes * Display wallet and proposal references clearly * Keep the timeline reusable for audit-style screens"</p>
      </header>
