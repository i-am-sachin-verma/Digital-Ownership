...
type SnapshotDashboardRange = "24h" | "7d" | "30d" | "90d";

const SnapshotDashboardSeries: SnapshotDashboardMetric[] = [
  { id: "m1", label: "Chain", value: 90, delta: 1.5, color: "neutral", format: "number" },
  { id: "m2", label: "Governance", value: 103, delta: 2.7, color: "positive", format: "currency" },
  { id: "m3", label: "Snapshot", value: 116, delta: 3.9, color: "negative", format: "percent" },
  { id: "m4", label: "Historical", value: 129, delta: 5.1, color: "neutral", format: "number" },
  { id: "m5", label: "Voter", value: 142, delta: 6.3, color: "neutral", format: "number" },
  { id: "m6", label: "Analytics", value: 155, delta: 7.5, color: "positive", format: "currency" },
...
function formatMetric(value: number, format: SnapshotDashboardMetric["format"]) {
  if (format === "currency") {
...
function MetricCard({
  ...
  metric: SnapshotDashboardMetric;
  points: number[];
}) {
  return (
...
function RangePill({
  ...
  value: SnapshotDashboardRange;
  active: boolean;
  onSelect: (value: SnapshotDashboardRange) => void;
}) {
  return (
    <button
      type="button"
...
export default function SnapshotDashboard() {
  const [range, setRange] = useState<SnapshotDashboardRange>("30d");
  ...
  const rows = useMemo(() => {
    ...
    return SnapshotDashboardSeries.map((metric, index) => ({
      ...metric,
  ...
  return (
    <section ref={anchorRef} className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Develop an On-Chain Governance Snapshot Dashboard with Historical Voter Analytics"</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          "Create a dashboard that summarizes historical participation, voter behavior, and proposal outcomes across governance rounds. * Show historical voting trends * Visualize participation by proposal type * Highlight repeat voters and inactive segments * Support period-based analytics views * Keep the visualization blocks reusable"
        </p>
      </header>
