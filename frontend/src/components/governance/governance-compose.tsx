...
export type GovernanceComposeStep = "draft" | "review" | "final";

export interface GovernanceComposeField {
  key: string;
  label: string;
  value: string;
  required?: boolean;
...
export interface GovernanceComposeState {
  step: GovernanceComposeStep;
  title: string;
...
const GovernanceComposeSections: GovernanceComposeField[] = [
  { key: "name", label: "Modular Name", value: "", required: true, helper: "A short label for the proposal." },
  { key: "scope", label: "Governance Scope", value: "", required: true, helper: "Explain the affected module or users." },
  { key: "notes", label: "Proposal Notes", value: "", helper: "Include assumptions, constraints, and rollout details." },
...
function createInitialState(componentName: string): GovernanceComposeState {
  return {
    step: "draft",
    title: componentName,
    fields: Object.fromEntries(
      GovernanceComposeSections.map((field) => [field.key, field.value])
    ),
    touched: {},
    errors: {},
    isSubmitting: false,
...
function reducer(state: GovernanceComposeState, action: GovernanceComposeAction): GovernanceComposeState {
  switch (action.type) {
    case "edit": {
...
export default function GovernanceCompose() {
  const [state, dispatch] = useReducer(reducer, createInitialState("Build a Modular Governance Proposal Composer with Multi-Step Drafting and Validation"));
  ...
  return (
    <div className="mx-auto grid max-w-5xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">{"Build a Modular Governance Proposal Composer with Multi-Step Drafting and Validation"}</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          {"Create a proposal creation interface that supports structured drafting, field validation, and review before submission. * Support multi-step proposal creation flow * Validate required metadata and proposal content * Allow draft saving and editing * Show live summary preview before submission * Keep the component reusable for different governance modules"}
        </p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
        <section className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            {GovernanceComposeSections.map((field) => (
              <FieldEditor
                key={field.key}
                field={field}
