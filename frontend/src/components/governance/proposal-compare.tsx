...
type ProposalCompareSide = "left" | "right";

type ProposalCompareChunk = {
  id: string;
  label: string;
  left: string;
  right: string;
...
const ProposalCompareChunks: ProposalCompareChunk[] = [
  {
    id: "overview",
...
export default function ProposalCompare() {
  ...
  const chunks = useMemo(() => {
    return ProposalCompareChunks.map((chunk) => {
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Design a Proposal Comparison Interface with Side-by-Side Amendment Review"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a comparison experience that lets users inspect multiple proposal versions, amendments, and text differences. * Compare original and updated proposal versions * Highlight added, removed, and modified content * Support scroll-linked version navigation * Preserve readability for long-form governance text * Keep the component modular for document workflows"</p>
      </header>
