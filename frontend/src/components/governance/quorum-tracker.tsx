...
type QuorumTrackerRange = "24h" | "7d" | "30d" | "90d";

const QuorumTrackerSeries: QuorumTrackerMetric[] = [
  { id: "m1", label: "Governance", value: 90, delta: 1.5, color: "neutral", format: "number" },
  { id: "m2", label: "Quorum", value: 103, delta: 2.7, color: "positive", format: "currency" },
  { id: "m3", label: "Tracker", value: 116, delta: 3.9, color: "negative", format: "percent" },
  { id: "m4", label: "Participation", value: 129, delta: 5.1, color: "neutral", format: "number" },
  { id: "m5", label: "Metrics", value: 142, delta: 6.3, color: "neutral", format: "number" },
  { id: "m6", label: "Outcome", value: 155, delta: 7.5, color: "positive", format: "currency" },
...
function formatMetric(value: number, format: QuorumTrackerMetric["format"]) {
  if (format === "currency") {
...
function RangePill({
  ...
  value: QuorumTrackerRange;
  active: boolean;
  onSelect: (value: QuorumTrackerRange) => void;
}) {
  return (
    <button
      type="button"
...
export default function QuorumTracker() {
  const [range, setRange] = useState<QuorumTrackerRange>("30d");
  ...
  const rows = useMemo(() => {
    ...
    return QuorumTrackerSeries.map((metric, index) => ({
      ...metric,
  ...
  return (
    <section ref={anchorRef} className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Create a Governance Quorum Tracker with Real-Time Participation Metrics and Outcome Forecasting"</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          "Build a panel that tracks quorum progress, participation trends, and likely proposal outcomes using frontend calculations. * Display quorum progress visually * Track yes, no, and abstain participation * Show estimated outcome based on current votes * Support responsive chart and stat layouts * Keep the logic isolated for reuse across proposal pages"
        </p>
      </header>
