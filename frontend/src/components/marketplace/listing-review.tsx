...
type ListingReviewAction =
  ...
  | { type: "status"; id: string; status: ListingReviewRecord["status"] }
  | { type: "note"; id: string; note: string };

const ListingReviewInitial: ListingReviewRecord[] = [
  { id: "a1", label: "Asset", amount: 12, status: "valid", selected: false, note: "Stable" },
  ...
  { id: "a4", label: "Screen", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Media", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: ListingReviewRecord[], action: ListingReviewAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: ListingReviewRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: ListingReviewRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: ListingReviewRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as ListingReviewRecord["status"])}
...
export default function ListingReview() {
  const [rows, dispatch] = useReducer(reducer, ListingReviewInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Develop an Asset Listing Review Screen with Media Preview, Metadata Checks, and Policy Flags"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a moderation-friendly listing review page that verifies asset media, metadata completeness, and policy warnings. * Preview listing media and metadata * Show validation and policy warning states * Support manual review actions * Preserve readability for long metadata fields * Keep the review layout reusable for admin tools"</p>
      </header>
