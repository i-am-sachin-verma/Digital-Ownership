...
const MarketplaceGridItems: MarketplaceGridItem[] = [
  { id: "i1", name: "Filter Alpha", price: 14.2, rarity: 94, chain: "Base", featured: true, listed: true },
  { id: "i2", name: "Nft Prime", price: 9.8, rarity: 81, chain: "Ethereum", featured: false, listed: true },
  { id: "i3", name: "Marketplace Core", price: 31.5, rarity: 76, chain: "Polygon", featured: true, listed: false },
  { id: "i4", name: "Grid Vault", price: 5.4, rarity: 88, chain: "Arbitrum", featured: false, listed: true },
  { id: "i5", name: "Dynamic Signal", price: 18.9, rarity: 90, chain: "Optimism", featured: true, listed: true },
  { id: "i6", name: "Sorting Drift", price: 22.3, rarity: 73, chain: "Base", featured: false, listed: false },
];

function sortItems(items: MarketplaceGridItem[], mode: "price" | "rarity" | "featured") {
  const copy = [...items];
...
export default function MarketplaceGrid() {
  ...
  const visible = useMemo(() => {
    return sortItems(
      MarketplaceGridItems.filter((item) => {
  ...
  const chains = useMemo(() => {
    return Array.from(new Set(MarketplaceGridItems.map((item) => item.chain)));
  }, []);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build a Multi-Filter NFT Marketplace Grid with Dynamic Sorting and Saved Views"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a marketplace listing page that supports complex filtering, sorting, and view persistence for collectible assets. * Filter by price, rarity, collection, and chain * Support multiple sort modes * Save and restore preferred views * Handle empty result states elegantly * Keep card and filter components reusable"</p>
      </header>
