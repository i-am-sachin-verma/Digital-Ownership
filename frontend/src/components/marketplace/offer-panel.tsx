...
type OfferPanelAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: OfferPanelRecord["status"] }
  | { type: "note"; id: string; note: string };

const OfferPanelInitial: OfferPanelRecord[] = [
  { id: "a1", label: "Marketplace", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Offer", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Negotiation", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Editable", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Terms", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: OfferPanelRecord[], action: OfferPanelAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: OfferPanelRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: OfferPanelRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: OfferPanelRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as OfferPanelRecord["status"])}
...
export default function OfferPanel() {
  const [rows, dispatch] = useReducer(reducer, OfferPanelInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build a Marketplace Offer Negotiation Panel with Editable Terms and Confirmation States"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a transaction-style offer interface inside the marketplace that supports custom offers, review, and acceptance states. * Allow editable price and expiry terms * Show offer summary before confirmation * Support pending, accepted, and rejected states * Provide clear validation for required fields * Keep the component reusable for buy and sell flows"</p>
      </header>
