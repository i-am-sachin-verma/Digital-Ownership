...
export type TransactionCheckoutStep = "draft" | "review" | "final";

export interface TransactionCheckoutField {
  key: string;
  label: string;
  value: string;
  required?: boolean;
...
export interface TransactionCheckoutState {
  step: TransactionCheckoutStep;
  title: string;
...
type TransactionCheckoutAction =
...
const TransactionCheckoutSections: TransactionCheckoutField[] = [
  { key: "name", label: "Step Name", value: "", required: true, helper: "A short label for the proposal." },
  { key: "scope", label: "Transaction Scope", value: "", required: true, helper: "Explain the affected module or users." },
  { key: "notes", label: "Checkout Notes", value: "", helper: "Include assumptions, constraints, and rollout details." },
...
function createInitialState(componentName: string): TransactionCheckoutState {
  return {
    step: "draft",
    title: componentName,
    fields: Object.fromEntries(
      TransactionCheckoutSections.map((field) => [field.key, field.value])
    ),
    touched: {},
    errors: {},
    isSubmitting: false,
...
function reducer(state: TransactionCheckoutState, action: TransactionCheckoutAction): TransactionCheckoutState {
  switch (action.type) {
    case "edit": {
...
function FieldEditor({
  ...
  field: TransactionCheckoutField;
  value: string;
  error?: string;
...
export default function TransactionCheckout() {
  const [state, dispatch] = useReducer(reducer, createInitialState("Create a Multi-Step Transaction Checkout with Review, Approval, and Final Confirmation"));
  ...
  return (
    <div className="mx-auto grid max-w-5xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">{"Create a Multi-Step Transaction Checkout with Review, Approval, and Final Confirmation"}</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          {"Build a transaction flow that guides users through review, approval, and final execution with strong state handling. * Support multi-step progression * Display a full transaction summary * Validate balances and permissions before confirmation * Handle success, pending, and failure states * Keep the flow reusable for transfers and marketplace actions"}
        </p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
        <section className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            TransactionCheckoutSections.map((field) => (
              <FieldEditor
                key={field.key}
                field={field}
