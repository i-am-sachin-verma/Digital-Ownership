...
export type LifecycleWizardStep = "draft" | "review" | "final";

export interface LifecycleWizardField {
  key: string;
  label: string;
  value: string;
  required?: boolean;
...
export interface LifecycleWizardState {
  step: LifecycleWizardStep;
  title: string;
...
const LifecycleWizardSections: LifecycleWizardField[] = [
  { key: "name", label: "Proposal Name", value: "", required: true, helper: "A short label for the proposal." },
  { key: "scope", label: "Lifecycle Scope", value: "", required: true, helper: "Explain the affected module or users." },
  { key: "notes", label: "Wizard Notes", value: "", helper: "Include assumptions, constraints, and rollout details." },
...
function createInitialState(componentName: string): LifecycleWizardState {
  return {
    step: "draft",
    title: componentName,
    fields: Object.fromEntries(
      LifecycleWizardSections.map((field) => [field.key, field.value])
    ),
    touched: {},
    errors: {},
    isSubmitting: false,
...
function reducer(state: LifecycleWizardState, action: LifecycleWizardAction): LifecycleWizardState {
  switch (action.type) {
    case "edit": {
...
export default function LifecycleWizard() {
  const [state, dispatch] = useReducer(reducer, createInitialState("Build a Proposal Lifecycle Wizard with Conditional Steps and Status Recovery"));
  ...
  return (
    <div className="mx-auto grid max-w-5xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">{"Build a Proposal Lifecycle Wizard with Conditional Steps and Status Recovery"}</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          {"Create a guided proposal workflow that adapts steps based on proposal state and supports recovery from incomplete drafts. * Show dynamic step progression * Resume incomplete proposal drafts * Allow step-specific validation and feedback * Support branching flows for special proposal types * Keep the wizard structure reusable across governance tools"}
        </p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_0.9fr]">
        <section className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            LifecycleWizardSections.map((field) => (
              <FieldEditor
                key={field.key}
                field={field}
