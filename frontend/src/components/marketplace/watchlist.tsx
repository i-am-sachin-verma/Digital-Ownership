...
const WatchlistItems: WatchlistItem[] = [
  ...
  { id: "i2", name: "Watchlist Prime", price: 9.8, rarity: 81, chain: "Ethereum", featured: false, listed: true },
  { id: "i3", name: "Persistent Core", price: 31.5, rarity: 76, chain: "Polygon", featured: true, listed: false },
  { id: "i4", name: "Favorites Vault", price: 5.4, rarity: 88, chain: "Arbitrum", featured: false, listed: true },
  { id: "i5", name: "Change Signal", price: 18.9, rarity: 90, chain: "Optimism", featured: true, listed: true },
  { id: "i6", name: "Highlighting Drift", price: 22.3, rarity: 73, chain: "Base", featured: false, listed: false },
];

function sortItems(items: WatchlistItem[], mode: "price" | "rarity" | "featured") {
  const copy = [...items];
...
export default function Watchlist() {
  ...
  const chains = useMemo(() => {
    return Array.from(new Set(WatchlistItems.map((item) => item.chain)));
  }, []);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Build a Marketplace Watchlist System with Persistent Favorites and Change Highlighting"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a watchlist interface that tracks favorited collections or assets and highlights meaningful changes over time. * Persist favorite items locally * Highlight price or volume changes * Support quick watchlist editing * Group items by category or priority * Keep the system modular for reuse across pages"</p>
      </header>
