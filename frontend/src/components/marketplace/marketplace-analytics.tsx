...
type MarketplaceAnalyticsMetric = {
  id: string;
  label: string;
  value: number;
  delta: number;
...
type MarketplaceAnalyticsRange = "24h" | "7d" | "30d" | "90d";

const MarketplaceAnalyticsSeries: MarketplaceAnalyticsMetric[] = [
  { id: "m1", label: "Marketplace", value: 90, delta: 1.5, color: "neutral", format: "number" },
  { id: "m2", label: "Analytics", value: 103, delta: 2.7, color: "positive", format: "currency" },
  { id: "m3", label: "Header", value: 116, delta: 3.9, color: "negative", format: "percent" },
  { id: "m4", label: "Kpis", value: 129, delta: 5.1, color: "neutral", format: "number" },
  { id: "m5", label: "Comparative", value: 142, delta: 6.3, color: "neutral", format: "number" },
  { id: "m6", label: "Period", value: 155, delta: 7.5, color: "positive", format: "currency" },
...
function formatMetric(value: number, format: MarketplaceAnalyticsMetric["format"]) {
  if (format === "currency") {
...
function MetricCard({
  ...
  metric: MarketplaceAnalyticsMetric;
  points: number[];
}) {
  return (
...
function RangePill({
  ...
  value: MarketplaceAnalyticsRange;
  active: boolean;
  onSelect: (value: MarketplaceAnalyticsRange) => void;
}) {
  return (
    <button
      type="button"
...
export default function MarketplaceAnalytics() {
  const [range, setRange] = useState<MarketplaceAnalyticsRange>("30d");
  ...
  const rows = useMemo(() => {
    ...
    return MarketplaceAnalyticsSeries.map((metric, index) => ({
      ...metric,
  ...
  return (
    <section ref={anchorRef} className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Create a Marketplace Analytics Header with Live KPIs and Comparative Period Switching"</h2>
        <p className="max-w-3xl text-sm text-slate-600">
          "Build a performance header that summarizes traffic, conversions, listings, and sales with time period switching. * Show key marketplace KPIs * Support daily, weekly, and monthly views * Compare current and previous periods * Handle responsive stat card arrangement * Keep metric rendering components reusable"
        </p>
      </header>
