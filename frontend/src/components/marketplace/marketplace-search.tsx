...
const MarketplaceSearchCatalog: MarketplaceSearchResult[] = [
  { id: "r1", label: "Marketplace", kind: "entity", detail: "Primary lookup target with fast access.", score: 96 },
  { id: "r2", label: "Search", kind: "collection", detail: "Recently opened item with strong relevance.", score: 91 },
  { id: "r3", label: "Autocomplete", kind: "wallet", detail: "Wallet-bound record with high confidence.", score: 88 },
  { id: "r4", label: "Recent", kind: "history", detail: "Cached item from recent activity.", score: 84 },
  { id: "r5", label: "Query", kind: "notification", detail: "Actionable alert with shortcuts.", score: 79 },
  { id: "r6", label: "Memory", kind: "route", detail: "Related search path and navigation hit.", score: 74 },
];
...
function rankResult(query: string, result: MarketplaceSearchResult) {
...
function ResultRow({
  ...
  item: MarketplaceSearchResult;
  active: boolean;
  onSelect: (item: MarketplaceSearchResult) => void;
}) {
  return (
    <button
      type="button"
...
export default function MarketplaceSearch() {
  ...
  const results = useMemo(() => {
    return MarketplaceSearchCatalog
  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Implement a Marketplace Search Experience with Autocomplete and Recent Query Memory"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a fast search interface that suggests collections, items, and creators while remembering recent searches. * Support live autocomplete suggestions * Save recent queries locally * Allow quick search refinement with keyboard input * Display ranked results by relevance * Keep the search module reusable across marketplace screens"</p>
      </header>
