...
const CategoryNavigatorItems: CategoryNavigatorItem[] = [
  { id: "i1", name: "Marketplace Alpha", price: 14.2, rarity: 94, chain: "Base", featured: true, listed: true },
  { id: "i2", name: "Category Prime", price: 9.8, rarity: 81, chain: "Ethereum", featured: false, listed: true },
  { id: "i3", name: "Navigator Core", price: 31.5, rarity: 76, chain: "Polygon", featured: true, listed: false },
  { id: "i4", name: "Hierarchical Vault", price: 5.4, rarity: 88, chain: "Arbitrum", featured: false, listed: true },
  { id: "i5", name: "Browsing Signal", price: 18.9, rarity: 90, chain: "Optimism", featured: true, listed: true },
  { id: "i6", name: "Smart Drift", price: 22.3, rarity: 73, chain: "Base", featured: false, listed: false },
];

function sortItems(items: CategoryNavigatorItem[], mode: "price" | "rarity" | "featured") {
  const copy = [...items];
...
export default function CategoryNavigator() {
  ...
  const visible = useMemo(() => {
    return sortItems(
      CategoryNavigatorItems.filter((item) => {
  ...
  const chains = useMemo(() => {
    return Array.from(new Set(CategoryNavigatorItems.map((item) => item.chain)));
  }, []);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Create a Marketplace Category Navigator with Hierarchical Browsing and Smart Facet Counts"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Build a browsing interface that helps users navigate categories, subcategories, and filter counts efficiently. * Support hierarchical category browsing * Show live counts for each facet * Allow collapsing and expanding groups * Maintain clarity for deep category trees * Keep the navigation component reusable across listings"</p>
      </header>
