...
type ListingComposerAction =
  | { type: "toggle"; id: string }
  | { type: "change"; id: string; amount: number }
  | { type: "selectAll"; selected: boolean }
  | { type: "status"; id: string; status: ListingComposerRecord["status"] }
  | { type: "note"; id: string; note: string };

const ListingComposerInitial: ListingComposerRecord[] = [
  { id: "a1", label: "Marketplace", amount: 12, status: "valid", selected: false, note: "Stable" },
  { id: "a2", label: "Listing", amount: 24, status: "idle", selected: false, note: "Pending" },
  { id: "a3", label: "Composer", amount: 31, status: "warning", selected: false, note: "Watch" },
  { id: "a4", label: "Bulk", amount: 4, status: "valid", selected: false, note: "Safe" },
  { id: "a5", label: "Select", amount: 19, status: "error", selected: false, note: "Fix" },
];

function reducer(state: ListingComposerRecord[], action: ListingComposerAction) {
  switch (action.type) {
    case "toggle":
...
function StatusPill({ value }: { value: ListingComposerRecord["status"] }) {
  const tone =
...
function RowEditor({
  ...
  row: ListingComposerRecord;
  onToggle: () => void;
  onAmount: (amount: number) => void;
  onStatus: (status: ListingComposerRecord["status"]) => void;
  ...
  return (
    <tr className={row.selected ? "bg-slate-50" : ""}>
      ...
      <td className="px-4 py-3">
        <select
          value={row.status}
          onChange={(event) => onStatus(event.target.value as ListingComposerRecord["status"])}
...
function NotesPanel({
  ...
  selected: ListingComposerRecord[];
...
export default function ListingComposer() {
  const [rows, dispatch] = useReducer(reducer, ListingComposerInitial);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Develop a Marketplace Listing Composer with Bulk Select and Batch Action Support"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a seller-side interface that allows bulk selection of listed items and batch actions such as relist, delist, or update price. * Support multi-item selection * Add batch action controls * Validate action eligibility per item * Show progress and result feedback * Keep the layout clean for large inventories"</p>
      </header>
