...
const NotificationCenterEvents: NotificationCenterEvent[] = [
  {
    id: "e1",
  ...
  {
    id: "e5",
    title: "Prioritization warning surfaced",
    kind: "alert",
    time: "5h ago",
    unread: true,
    archived: false,
...
function kindTone(kind: NotificationCenterEvent["kind"]) {
  switch (kind) {
    case "proposal":
...
function EventBadge({ kind }: { kind: NotificationCenterEvent["kind"] }) {
  return (
...
export default function NotificationCenter() {
  const [filter, setFilter] = useState<NotificationCenterEvent["kind"] | "all">("all");
  const [showArchived, setShowArchived] = useState(true);
  const [items, setItems] = useState(NotificationCenterEvents);
  ...
  return (
    <section className="mx-auto grid max-w-5xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Create a Governance Notification Center with Alert Prioritization and Read State Management"</h2>
        <p className="max-w-3xl text-sm text-slate-600">"Build a notification interface for proposal updates, vote reminders, execution alerts, and delegate changes. * Prioritize urgent governance alerts * Support unread and read state toggles * Group related notifications together * Allow quick jump to proposal or vote pages * Keep the notification item component reusable"</p>
      </header>
