...
const CreatorCommerceItems: CreatorCommerceItem[] = [
  { id: "i1", name: "Creator Alpha", price: 14.2, rarity: 94, chain: "Base", featured: true, listed: true },
  { id: "i2", name: "Profile Prime", price: 9.8, rarity: 81, chain: "Ethereum", featured: false, listed: true },
  { id: "i3", name: "Commerce Core", price: 31.5, rarity: 76, chain: "Polygon", featured: true, listed: false },
  { id: "i4", name: "Featured Vault", price: 5.4, rarity: 88, chain: "Arbitrum", featured: false, listed: true },
  { id: "i5", name: "Items Signal", price: 18.9, rarity: 90, chain: "Optimism", featured: true, listed: true },
  { id: "i6", name: "Sales Drift", price: 22.3, rarity: 73, chain: "Base", featured: false, listed: false },
];

function sortItems(items: CreatorCommerceItem[], mode: "price" | "rarity" | "featured") {
  const copy = [...items];
...
export default function CreatorCommerce() {
  ...
  const visible = useMemo(() => {
    return sortItems(
      CreatorCommerceItems.filter((item) => {
  ...
  const chains = useMemo(() => {
    return Array.from(new Set(CreatorCommerceItems.map((item) => item.chain)));
  }, []);
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-6 p-6">
      <header className="grid gap-2">
        ...
        <h2 className="text-2xl font-semibold text-slate-900">"Design a Creator Profile Commerce View with Featured Items and Sales History"</h2>
        <p className="max-w-4xl text-sm text-slate-600">"Create a creator-centric marketplace page that highlights listings, sold items, and collection performance. * Show featured and active listings * Display creator sales history trends * Support tabbed navigation across content groups * Handle loading skeletons and error states * Keep profile sections composable"</p>
      </header>
