...
contract SovereignIdentityRegistry {
    ...
    event IdentityUpdated(address indexed user, bytes32 oldHash, bytes32 newHash);
    event IdentityRevoked(address indexed user, bytes32 oldHash);
    ...
    function updateIdentity(bytes32 _newIdentityHash) external {
        ...
        if (_newIdentityHash == bytes32(0)) revert InvalidIdentity();
        
        bytes32 oldHash = identity.hash;
        identity.hash = _newIdentityHash;

        emit IdentityUpdated(msg.sender, oldHash, _newIdentityHash);
    }
    ...
    function revokeIdentity() external {
        ...
        emit IdentityRevoked(msg.sender, oldHash);
    }
